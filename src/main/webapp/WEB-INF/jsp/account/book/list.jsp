...
<head>
    ...
    <link href="<spring:url value="/css/timeline.css"/> " rel="stylesheet">
    ...
    <div class="container">
        <div class="row">
            <div class=".col-xs-12 .col-md-8">
                ...
                <h1 class="text-center">My passenger's journeys</h1>
                <c:forEach items="${journeys}" var="journey">
                <c:choose>
                <c:when test="${journey.id == journeyBooked.id}">
                <article class="container well" style="background-color: #CCFFCC;">
                    </c:when>
                    <c:otherwise>
                                    ...
                                    <div class="col-md-6 no-pad">
                                        <div class="user-pad">
                                            ...
                                            <a href="${pageContext.request.contextPath}/account/view/${journey.getCreator().getId()}">
                                                <button type="button" class="btn btn-labeled btn-info">Voir le
                                                    profil
                                                </button>
                                            </a>
                                        </div>
                                        ...
                                        <div class="user-image">
                                            <c:choose>
                                                <c:when test="${not empty journey.getCreator().getPicturePath()}">
                                                    <img src="<spring:url value="/users/photos/${journey.getCreator().getPicturePath()}"/>"
                                                         alt="profil photo" class="img-circle pull-right"/>
                                                </c:when>
                                                <c:otherwise>
                                                    <img src="<spring:url value="/users/photos/man.gif"/>"
                                                         alt="profil photo" class="img-circle pull-right"/>
                                                </c:otherwise>
                                            </c:choose>
                                        </div>
                                ...
                                <div class="user-menu-content">
                                    <div class="col-md-10 no-pad">
                                        <div class="row bs-wizard" style="border-bottom:0;">
                                            <c:set var="destStep" value="false"/>
                                            <c:forEach items="${journey.getSteps()}" var="step">
                                            <c:choose>
                                            <c:when test="${destStep == true}">
